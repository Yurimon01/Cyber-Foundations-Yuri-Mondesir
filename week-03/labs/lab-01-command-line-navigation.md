...
```
rm [-rf] FILE... - remove files. -r for directories, -f to force.
```
...
```
This was printed in linux. I would say this because in Linux every user gets their own personal space called / home /. 
```
...
```
The move then check matters because we are still new to command line, this way we as analysts can confirm that the next changes we are making are in the correct location.
```
...
```
I believe this habit matters because IT/Cybersecurity is so vast, and current professionals still get things spelt wrong or get confused from time to time. Looking up a command instead of guessing, provides an assurance that the correct command is what is being implemented.
```
...
```
What surprised me the most was how easy it was to get the hang of the first few commands. It truly does feel like you're sifting through folders, going back, looking inside and the verbiage of the commands directly correlate to how it feels to look through files for something. "Ls" it would make sense to list out or open everything that is in a folder in order to look for what you need. "cd" or change directory is straight forward, when you want to change folders, you'd move so accordingly.
```
