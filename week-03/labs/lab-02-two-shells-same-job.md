...
```
/home/agent/archive/incident-07>
...
```
PS /home/agent/archive/incident-07> New-Item investigation-notes.txt -ItemType File
```
...
```
since there is no < cp > in Powershell here are the commands that were used. 

PS /home/agent/archive/incident-07> Copy-Item investigation-notes.txt investigation-notes-backup.txt
```
...
| 2. Look around | ls | ls |
| 3. Move into a folder | cd archive | cd archive |
| 4. Peek inside a file | cat access-log.txt | cat access-log.txt |
| 5. Create + back up your note | touch investigation-notes.txt cp investigation-notes.txt investigation-notes-backup.txt | PS /home/agent/archive/incident-07> New-Item investigation-notes.txt -ItemType File PS /home/agent/archive/incident-07> Copy-Item investigation-notes.txt investigation-notes-backup.txt |
...
```
In the Windows Powershell command line, the contents of a folder are represented differently. We get two columns that represent the "mode" and the "name". I had to search up what <Mode> meant. It points to the type of content when listed.
```
...
```
Well, I might not be the best example, as I used many of the same commands that were aliases. But it was clear that the file system had identical trees, because we saw the same named folders and files as we went down the tree. "Cd archive" in both Linux and PowerShell produced the result of the two folder names "incident-07" and "incident-42".
```
...
```
I will say hands down Linux felt more comfortable. I believe it was way more intuitive than Windows PowerShell as the commands were short and easy to remember. Meaning way less opportunities for the command to not work due to spelling issues or forgetting a hyphen.
```
...
```
I didn't use any slash styles because I used mostly aliases. However, I would say when creating a file in PowerShell the case-sentivity did come up. This was an issue for me as I would type to fast and have to re-do the command.
```
