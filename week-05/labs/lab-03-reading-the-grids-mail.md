...
```
The source column is telling me who is speaking!
```

What the Destination column tells you:

```
The destination column is telling which server the source is speaking to!
```

What the Protocol column tells you:

```
The protocol column is telling me what type of communication rules are the source and destination talking through.
```

What the Info column tells you:

```
The column more clearly breaks what is being communicated and or done during the communication.
```
...
```
I would say my workstations IP address is 10.20.5.42, and I worked that out by seeing that, this was the station that "initiated" the most conversations.
```
...
```
The packet number of the query was number 1 and it was asking for the "foundry-archive.grid.local".
```

The source and destination addresses of the query — who asked, and who was asked:

```
The source was ivys-workstation and who was the source asked was the grid-dns (or the domain name system )
```
...
```
Packet number 2 returned its Host Address - 10.20.5.20
```
...
```
The port number DNS used is 53.
```
...
```
Well dig just showed me the result of the DNS lookup. While the packet view is showing us what packets of communication are actually being sent in real time.
```
...
```
Packet number 7 and the source port is starting a conversation with the packet to the right (X to Y)
```

The SYN-ACK — packet number and direction:

```
Packet Number 8 it saying yes we can communicate, so Y is responding to X.
```

The ACK — packet number and direction:

```
And from there packet 9 is saying thank you, where X again is responding to Y.
```
...
```
51514 - is the odd-looking door of my server.
443 - is the well-known door, the standard door for "HTTPS".
```
...
```
I would now know the internal staff code FOUNDRY-2026-STOREROOM because it was sent in readable HTTP. I am not sure what that means exactly. But for someone who does know what it means and shouldn't have access to it this can create an issue.
```
...
```
This detail pane is showing me what can be a hashed message. Either way the message is not readable to me.
```
...
```
You can tell what the source and the destination server is, as well as what protocol is being used to communicate
```
...
```
A filter is a view because you're not completely removing the other packets by searching the specific type you want to zero in on. If I forgot that a filter was applied, I could think certain network traffic never occurred when it is actually just hidden. This could cause me to make the wrong conclusion because I would be analyzing only part of the evidence.
```
...
```
So dig is useful for finding out wether an IP address returns when a name is entered. But the packet view lets me see the actual request and response between my workstation and the DNS server and what is being said.
 I would specifically use the packet view if I needed to investigate where communication was failing or confirm whether requests and responses were actually being sent.
```
...
```
The handshake is confirming a consensus of conversation. Those three packets accomplished to let each other know that now they will be speaking and in the future the issues of conversation will not be between these 2 servers. A connection would be less reliable without this handshake because we are not sure if he server is agreement to respond or that the original server even recieved that acknowledgement.
```
...
```
Packet 10 seems to have some type of protection because I was not able to read what was being communicated, while packet 14 was completely readable. I think the difference is that packet 10 was using TLS and port 443, while packet 14 was using HTTP and port 80. This matters because packet 14 contained a staff code that I probably was not meant to know. I feel that I observed a code that is meant for a staff's badge or something like that.
```
