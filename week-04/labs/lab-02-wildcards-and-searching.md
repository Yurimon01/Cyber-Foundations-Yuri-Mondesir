...
```
I noticed roughly 13 files in the archive folder. I have also noticed the family names of invoices, and I am not sure if this counts but there are repetitions of the word "march" and ".log".
```
...
```
If these times are in military time as I presume, the denied badge at 2 AM in a "store-room" sounds a bit off. 
```
...
```
the owner/user, the selected group and others have read and modify access of this text file.
```
...
```
morgan@foundry:/home/morgan/archive$ ls -l meeting-recap.txt
...
door-access.log  east-access.log  inv-april.txt  inv-february.txt  inv-january.txt  inv-march-supplement.txt  inv-march.txt  meeting-recap.txt  notes-march-meeting.txt  supply-list.txt  west-access.log
morgan@foundry:/home/morgan/archive$ ls -l meeting-recap.txt
-rw----rw- 1 morgan foundry   129 meeting-recap.txt
morgan@foundry:/home/morgan/archive$ chmod o-rw meeting-recap.txt
morgan@foundry:/home/morgan/archive$ ls -l meeting-recap.txt
-rw------- 1 morgan foundry   129 meeting-recap.txt
morgan@foundry:/home/morgan/archive$
...
```
If you don't use the ls command prior to, you do not know what is actually happening or what changed. This is dangerous when we start using rm < remove/ delete > in our patters because if we are not sure "what" we are deleting we can create permanent damage to something we don't even have knowledge of to trace back.
```
...
```
I believe this is important for a number of reasons. But one reason is because it allows you to be more efficient with your time in securing saftey. If an attacker was going around and trying to get into a building, shifting through all the logs to take more time. Thus, giving more time for the attacker to strike again or get away with whatever damage they were seeking to do.
```
...
```
These two skills would not be enough on their own as they only work together when it comes to ensuring security. If you go in an audit a file's permissions but do not know what the contents of that file are, you could be wasting your time and you have no aim for what you are trying to protect. On the flip side, knowing the contents of a sensitive file, without being able to do anything about it is just as useless. 
```
...
```
I think in the professional industry, the habit of checking, acting, and then checking again is what scales up. When it comes to auditing a system or simply checking on things, making sure you know what you're dealing with and checking that your action actually worked is very useful in steering a ship away from catastrophe.
```
...
![Archive investigation — find, check, lock down](https://raw.githubusercontent.com/Yurimon01/Cyber-Foundations-Yuri-Mondesir/refs/heads/main/assets/screenshots/week-04/cli-search-investigation.png)
