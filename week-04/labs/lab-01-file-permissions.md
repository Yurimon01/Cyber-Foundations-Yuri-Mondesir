...
```
(For the file titled badges-codes.txt, the owner/user can read and modify the file. The specific group and others both only have reading access.)
```
...
This is obviously not good as every group has the access to modify the master-inventory file.
```
...
morgan@foundry:/home/morgan/badge-office$ ls -l
morgan@foundry:/home/morgan/badge-office$ chmod o-r master-inventory.txt
morgan@foundry:/home/morgan/badge-office$ ls -l
morgan@foundry:/home/morgan/badge-office$ chmod o-w master-inventory.txt
morgan@foundry:/home/morgan/badge-office$
...
```
morgan@foundry:/home/morgan/badge-office$ chmod g-r badge-codes.txt
morgan@foundry:/home/morgan/badge-office$ ls -l
...
```
-rw------- 1 morgan foundry    82 badge-codes.txt
-rwxr--r-- 1 morgan foundry    66 cleanup.sh
-rw-r----- 1 morgan foundry   151 master-inventory.txt
-rw-r--r-- 1 morgan foundry    66 shift-notes.txt
-rw-r--r-- 1 morgan foundry    49 supply-list.txt
...
```
The owner/user has read and write access to the shift-notes.txt file.
```
...
```
The other audience is the most important one to audit as this is considered everyone else. If we were looking at the permissions of who has access to the security-codes.txt file, you would want everyone else who is not "the user" or a "specified group" to have access to information that they can exploit. 
```
...
```
The check before the chmod, allows you see to what you are changing and what permissions you are changing. You can't know what you changed if you don't know what was originally there. And the check after allows you to confirm that the permission you wished to change actually changed and that you didn't input the wrong modifications.
```
...
```
One of the files I will be analyzing is the others group having any access to the badge-codes.txt. In the terms of least privilege we removed the others group access to read or modify this file. If we think about there is no reason for a group that is not the user, creator, admin, or security to need access to search sensitive information. Someone who is pretending to be a worker and stole someone's badge could get into a place they shouldn't have just by being able to READ that file.
```
...
```
I believe with Windows ACL you can actually give permissions to specific people. This could be an option rather than throwing people in a group. This is useful for specificity but probably causes there to be too much to manage when the number of specific people needing access increases maybe.
```
